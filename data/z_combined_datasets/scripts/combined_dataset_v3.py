...
stations_gdf["nearest_transit_stop_dist_m"] = stations_gdf.geometry.apply(
    lambda station_geom: stops_gdf.distance(station_geom).min()
)

scores_and_coords = scores_and_coords.merge(
    stations_gdf[["id", "nearest_transit_stop_dist_m"]],
    on="id",
    how="left",
)

scores_and_coords["nearest_transit_stop_dist_m"] = scores_and_coords[
...
# %%
# -----------------------------
# Distance to nearest park (meters)
# -----------------------------
stations = scores_and_coords.copy()
parks_polys = parks.copy()
...
scores_and_coords["nearest_park_dist_m"] = scores_and_coords[
    "nearest_park_dist_m"
].round(2)
...
# %%
# -----------------------------
# Entertainment and tourism within 275m
# -----------------------------
stations = scores_and_coords.copy()
retail_pts = retail.copy()
...
retail_ent = retail_gdf[retail_gdf["type"].isin(entertainment_types)].copy()
joined_ent = gpd.sjoin(retail_ent, stations_buffer, how="inner", predicate="within")

ent_counts = (
    joined_ent.groupby("id").size().rename("entertainment_nearby").reset_index()
)

scores_and_coords = scores_and_coords.merge(ent_counts, on="id", how="left")
scores_and_coords["entertainment_nearby"] = (
    scores_and_coords["entertainment_nearby"].fillna(0).astype(int)
)
...
retail_tour = retail_gdf[retail_gdf["type"].isin(tourism_types)].copy()
joined_tour = gpd.sjoin(retail_tour, stations_buffer, how="inner", predicate="within")

tour_counts = joined_tour.groupby("id").size().rename("tourism_nearby").reset_index()

scores_and_coords = scores_and_coords.merge(tour_counts, on="id", how="left")
scores_and_coords["tourism_nearby"] = (
    scores_and_coords["tourism_nearby"].fillna(0).astype(int)
)
...
dist_features["avg_stations_dist_3_nearest_m"] = dist_no_self.apply(
...
# %%
# -----------------------------
# Distance to nearest dining hall (meters)
# -----------------------------
stations = scores_and_coords.copy()
dining_halls_pts = dining_halls.copy()
...
dining_halls_gdf = gpd.GeoDataFrame(
    dining_halls_pts,
    geometry=gpd.points_from_xy(dining_halls_pts["lon"], dining_halls_pts["lat"]),
    crs="EPSG:4326",
)

# project to meters
stations_gdf = stations_gdf.to_crs(epsg=3857)
dining_halls_gdf = dining_halls_gdf.to_crs(epsg=3857)

# compute nearest dining hall distance for each station
stations_gdf["nearest_dining_hall_dist_m"] = stations_gdf.geometry.apply(
    lambda station_geom: dining_halls_gdf.distance(station_geom).min()
)

# merge back
scores_and_coords = scores_and_coords.merge(
    stations_gdf[["id", "nearest_dining_hall_dist_m"]],
    on="id",
    how="left",
)

scores_and_coords["nearest_dining_hall_dist_m"] = scores_and_coords[
    "nearest_dining_hall_dist_m"
].round(2)

# %%
# -----------------------------
# Distance to nearest dorm (meters) + nearest dorm population
# -----------------------------
stations = scores_and_coords.copy()
dorms_pts = dorms.copy()
...
dorms_gdf = gpd.GeoDataFrame(
    dorms_pts,
    geometry=gpd.points_from_xy(dorms_pts["lon"], dorms_pts["lat"]),
    crs="EPSG:4326",
)
...
# for each station, find nearest dorm and pull both distance + population
def get_nearest_dorm_info(station_geom):
    distances = dorms_gdf.geometry.distance(station_geom)
    nearest_idx = distances.idxmin()

    return pd.Series(
        {
            "nearest_dorm_dist_m": distances.loc[nearest_idx],
            "nearest_dorm_pop": dorms_gdf.loc[nearest_idx, "population"],
        }
    )


stations_gdf[["nearest_dorm_dist_m", "nearest_dorm_pop"]] = stations_gdf.geometry.apply(
    get_nearest_dorm_info
)
...
scores_and_coords["nearest_dorm_dist_m"] = scores_and_coords[
    "nearest_dorm_dist_m"
].round(2)

# %%
# -----------------------------
# Dorm population within 500m
# -----------------------------
stations = scores_and_coords.copy()
dorms_pts = dorms.copy()
...
stations_buffer["geometry"] = stations_buffer.geometry.buffer(500)

joined = gpd.sjoin(dorms_gdf, stations_buffer, how="inner", predicate="within")

dorm_pop_500 = (
    joined.groupby("id")["population"]
    .sum()
    .rename("dorm_pop_within_500m")
    .reset_index()
)

scores_and_coords = scores_and_coords.merge(dorm_pop_500, on="id", how="left")
scores_and_coords["dorm_pop_within_500m"] = (
    scores_and_coords["dorm_pop_within_500m"].fillna(0).astype(int)
)

# %%
scores_and_coords[["name", "nearest_dorm_dist_m", "nearest_dorm_pop"]].head()

# %%
scores_and_coords[["name", "nearest_dining_hall_dist_m", "nearest_dorm_dist_m"]].head()
...
dining_halls.head()
# name	lat	lon
# 0	Kins Dining	30.290512	-97.739781
# 1	J2 Dining	30.283019	-97.736825
# 2	Jester City Limits	30.283002	-97.736818

dorms.head()
# name	population	lat	lon
# 0	Andrews Hall	132	30.288325	-97.739849
# 1	Blanton Hall	270	30.288565	-97.739332
# 2	Carothers Hall	130	30.288643	-97.740390
# 3	Creekside Hall	190	30.289100	-97.730500
# 4	Duren Hall	588	30.290200	-97.741600


# %%
scores_and_coords.to_csv(
    "../../cleaned/combined_datasets/v3/combined_dataset_v3.csv", index=False
)
...
X_scaled.to_csv("../../cleaned/combined_datasets/v3/ml_dataset_v3.csv", index=False)
